...
class CustomerController: PasswordRetriever{
    ...
    /*
    fun retrieveCustomerFromSet(userId: String): Map<String,String>?{
        for(customer in Database.customers){
            if(customer.userId == userId){
                return mapOf("userName" to customer.userName,"dateOfBirth" to customer.dateOfBirth.toString(),"address" to customer.address,"gender" to customer.gender.toString())
            }
        }
        return null
    }
