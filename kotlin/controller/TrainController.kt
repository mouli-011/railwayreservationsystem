        ...
        for (coach in coachDetails) {
            coaches.add(
                Coach(
                    ...
                    coach["farePerStation"]?.toDouble() ?: 0.0
                )
            )
        }
        return coaches
    ...
    fun updateCoachAvailability(trainNumber: Int,coachName: CoachName,totalNoOfTickets: Int): Set<Coach>{
        ...
        for(train in Database.trains){
            if(train.trainNumber == trainNumber){
               for(coach in train.coaches){
                   if(coach.coachName == coachName){
                       updatedCoaches.add(Coach(coach.coachName,coach.noOfCompartments,coach.totalSeatsInCoach,coach.availableSeatsInCoach-totalNoOfTickets,coach.farePerStation))
                   }
                   else{
        ...
        for(train in Database.trains){
            if(train.trainNumber == trainNumber){
                with(TrainView){
                    ...
                    train.coaches.forEach { displayCoach(it.coachName,it.noOfCompartments,it.totalSeatsInCoach,it.availableSeatsInCoach,it.farePerStation) }
                }
            }
        }
    }
